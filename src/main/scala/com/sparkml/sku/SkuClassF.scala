...
//    val fileRDD = spark.sparkContext.textFile("D:\\logs\\allskucate.txt")
//
...
//    val otherRDD = spark.sparkContext.textFile("D:\\logs\\bother.csv")

    //val fileRDD= brandRDD++bookRDD


    val splitRDD = brandRDD.map(line => {
      try {
        //按照之前处理文本添加的奇异分隔符进行分割
        val splitWords = line.split("\t")
        //对文本中存在的一些无用符号作一些清除
...
//    val otherSplitRDD = otherRDD.map(line => {
//      try {
//        //按照之前处理文本添加的奇异分隔符进行分割
//        val splitWords = line.split("\t")
//        //对文本中存在的一些无用符号作一些清除
...
//    })
    ...
    //val splitRDD=brandSplitRDD++otherSplitRDD
    ...
    model.write.overwrite().save("/tmp/spark-bayes-cate4-Sku-model")
    //使用训练好的模型预测测试集
...
//    unResult.write.csv("D:\\logs\\labelbrand4")
